...
public:
    int maxConsecutive(int bottom, int top, vector<int>& special) {
        ...
        if(sz==1){
            int curcnt=special[0]-bottom;
            fnans=max(fnans, curcnt);
            int curcnt_=top-special[0];
            fnans=max(fnans, curcnt_);
            return fnans;
        }
        for(int i=0; i<sz; ++i){
            if(i==0){
                if(special[i]!=bottom){
                    int curcnt=special[i]-bottom;
                    fnans=max(fnans, curcnt);
                }
            } else if(i==sz-1){
                if(special[i]!=top){
                    int curcnt=top-special[i];
                    fnans=max(fnans, curcnt);
                }
            }
            if(i-1>=0){
                int curcnt=(special[i]-special[i-1])-1;
                fnans=max(fnans, curcnt);   
            }
        }
        return fnans;
