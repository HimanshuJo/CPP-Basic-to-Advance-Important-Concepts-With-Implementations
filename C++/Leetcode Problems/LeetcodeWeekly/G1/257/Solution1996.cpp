...
than this character's attack and defense levels. More formally, a character i is said to be weak if there 
exists another character j where attackj > attacki and defensej > defensei.
...
we need both attack and defence of the player should be less than the other player then only we can 
say the later person is weaker
...
if the array is sorted on one parameter we only need to worry about other. 

As the array is sorted the greater index cant be weaker than then smaller index i.e if i > j 
i cant be weaker than j (as the array is sorted)

so lets iterate from back and store the maximum element we 
encounter(as if we found a larger element(value of second parameter) on right it will be weaker as the second 
element is less and first one was already less because of sorting so both strenght and defence is weak)
