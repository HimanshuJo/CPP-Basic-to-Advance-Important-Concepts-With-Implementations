...
public:
    ...
    bool isValid(int nwx, int nwy, int rws, int cols, vector<vector<char>>&grd, vector<vector<int>>&seen){
        return nwx>=0&&nwy>=0&&nwx<=rws-1&&nwy<=cols-1&&grd[nwx][nwy]!='+'&&seen[nwx][nwy]==-1;
    }
    
    int nearestExit(vector<vector<char>>& maze, vector<int>& entrance) {
        int rws=maze.size(), cols=maze[0].size();
        vector<vector<char>>grd=maze;
        for(int i=0; i<cols; ++i){
            if(maze[0][i]=='.') grd[0][i]='#';
            if(maze[rws-1][i]=='.') grd[rws-1][i]='#';
        }
        for(int i=0; i<rws; ++i){
            if(maze[i][0]=='.') grd[i][0]='#';
            if(maze[i][cols-1]=='.') grd[i][cols-1]='#';
        }
        grd[entrance[0]][entrance[1]]='.';
        ...
        while(!q.empty()){
            ...
            while(sz--){
                ...
                q.pop();
                if(grd[curr.first][curr.second]=='#') return ans;
                for(auto &vals: dir){
                    int nwx=vals[0]+curr.first, nwy=vals[1]+curr.second;
                    if(isValid(nwx, nwy, rws, cols, grd, seen)){
                        q.push({nwx, nwy});
                        seen[nwx][nwy]=1;
                    }
                }
