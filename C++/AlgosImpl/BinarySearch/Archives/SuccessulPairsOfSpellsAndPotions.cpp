...
public:
    vector<int> successfulPairs(vector<int>& spells, vector<int>& potions, long long success) {
        ...
        vector<int>res(sz, 0);
        for(int i=0; i<sz; ++i){
            long long numNeeded=success/spells[i];
            while(true){
                if((long long)numNeeded*spells[i]<success){
                    numNeeded++;
                } else break;
            }
            auto it=lower_bound(potions.begin(), potions.end(), numNeeded);
            int idx=abs(it-potions.begin());
            int curres=sz2-idx;
            res[i]=curres;
        }
        return res;
...
public:
    ...
    int binarySearch(vector<int>&potions, long long numNeeded){
        int left=0, right=potions.size()-1;
        int ans=INT_MAX;
        while(left<=right){
            int mid=left+(right-left)/2;
            if(potions[mid]>=numNeeded){
                ans=mid;
                right=mid-1;
            } else{
                left=mid+1;
    ...
    vector<int> successfulPairs(vector<int>& spells, vector<int>& potions, long long success) {
        ...
        for(int i=0; i<sz; ++i){
            ...
            int idx=binarySearch(potions, numNeeded);
