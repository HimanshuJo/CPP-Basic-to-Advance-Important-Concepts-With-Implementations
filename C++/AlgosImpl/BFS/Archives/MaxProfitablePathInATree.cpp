...
public:
    ...
    void bfs1(vector<vector<int>>&gr, vector<int>&parent, vector<int>&dist){
        queue<int>q;
        q.push(0);
        set<int>seen;
        seen.insert(0);
    ...
    int bfs2(vector<vector<int>>&gr, vector<int>&amount){
        queue<int>q;
        q.push(0);
        int ans=INT_MIN;
        while(!q.empty()){
            ...
            int sz=q.size();
            for(auto nei: gr[curr]){
                if(amount[nei]==INT_MIN) continue;
                amount[nei]+=amount[curr];
                q.push(nei);
            }
            if(sz==q.size()){
                ans=max(ans, amount[curr]);
            }
            amount[curr]=INT_MIN;
        }
        return ans;
    ...
    int mostProfitablePath(vector<vector<int>>& edges, int bob, vector<int>& amount) {
        int maxNodes=amount.size();
        vector<vector<int>>gr(maxNodes);
        for(auto &vals: edges){
            gr[vals[0]].push_back(vals[1]);
            gr[vals[1]].push_back(vals[0]);
        }
        vector<int>parent(maxNodes), dist(maxNodes);
        bfs1(gr, parent, dist);
        int bobstr=bob, bobcurdist=0;
        while(bobstr){
            if(dist[bobstr]>bobcurdist){
                amount[bobstr]=0;
            } else if(dist[bobstr]==bobcurdist){
                amount[bobstr]/=2;
            }
            bobcurdist++, bobstr=parent[bobstr];
        }
        return bfs2(gr, amount);
    }
};