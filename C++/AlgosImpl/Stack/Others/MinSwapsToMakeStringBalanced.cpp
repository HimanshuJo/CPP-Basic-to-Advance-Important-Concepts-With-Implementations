...
/*
First of all it is guaranteed that total number of '[' brackets is equal to total number of ']' brackets.
So, for every bracket element we are sure that its counterpart exist.

Also, to keep string balanced, at every i
count of '[' >= count of ']'

If at any moment count of ']' exceeds count of '[', this is the time when we need to do swapping.
...
balance = 0 , initially
When we see a opening bracket, we increment balance by one.
...
this will balance the opening bracket that occurred previously. ']' -> -1
...
Now, the main catch where we need to swap. When number of closing brackets exceed the already 
occurred opening brackets. This is where we need to do swapping.

when balance < 0, or balance == -1 , swap this element with first occurring '[' from last.
