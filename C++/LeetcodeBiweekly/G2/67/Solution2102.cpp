...
string among all locations and score is an integer. 
...
For example, when the system is queried for the 4th time, it returns the 4th best 
location of all locations already added.
...
string get() Queries and returns the ith best location, where i is the number of 
times this method has been invoked (including this invocation).
...
struct cmp {
    bool operator()(const tuple<int, string, int>&A, const tuple<int, string, int>&B) const {
...
public:
    ...
    multimap<tuple<int, string, int>, string, cmp>mmp;
    vector<string>ans;
    int counter=0;
    ...
    void add(string name, int score) {
        mmp.insert(pair<tuple<int, string, int>, string>(make_tuple(score, name, counter), name));
        int idx=0;
        multimap<tuple<int, string, int>, string>::iterator itr;
        for(itr=mmp.begin(); itr!=mmp.end(); ++itr) {
            tuple<int, string, int> temp=itr->first;
            ans[idx++]=itr->second;
        }
    }
...
public:
    ...
    priority_queue<pair<int, string>, vector<pair<int, string>>, custComp1>pq1;
    priority_queue<pair<int, string>, vector<pair<int, string>>, custComp2>pq2;
    ...
    void add(string name, int score) {
        if(!pq1.empty()&&(pq1.top().first<score||(pq1.top().first==score?pq1.top().second>name:pq1.top().first<score))){
            pair<int, string>curr=pq1.top();
            pq1.pop();
            pq2.push(curr);
            pq1.push({score, name});
        } else{
            pq2.push(make_pair(score, name));
        }
    }
    
    string get() {
        pair<int, string>curr=pq2.top();
        pq2.pop();
        pq1.push(curr);
        return curr.second;
    }
...
public:
    ...
    void add(string name, int score) {
        auto [newit, b]=st.insert({-score, name});
        if(it==st.end()||*newit<*it) advance(it, -1);
    }
