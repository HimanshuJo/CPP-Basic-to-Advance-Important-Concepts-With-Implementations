...
For each lantern, you can choose its power — an integer between 0 and d (inclusive). 
A lantern with coordinate x illuminates the point of interest with coordinate y if |x−y| is 
less than or equal to the power of the lantern.

A way to choose the power values for all lanterns is considered valid if every point of interest 
is illuminated by at least one lantern.
...
Input
The first line contains three integers d, n and m (4≤d≤3⋅105; 1≤n≤2⋅105; 1≤m≤16) — the size of the segment, 
the number of lanterns and the number of points of interest, respectively.
