...
public:
    vector<int> arrayChange(vector<int>& nums, vector<vector<int>>& operations) {
        ...
        for(int i=0; i<sz; ++i){
            replace(nums.begin(), nums.end(), operations[i][0], operations[i][1]);
        }
        return nums;
    }
};
...
public:
    vector<int> arrayChange(vector<int>& nums, vector<vector<int>>& operations) {
        ...
        for(int i=0; i<sz; ++i){
            nums[temp[operations[i][0]]]=operations[i][1];
            // temp[operations[i][0]] currently holds operations[i][0]'s index
            // updating the new value also with the same index
            temp[operations[i][1]]=temp[operations[i][0]];
        }
        return nums;
    }
};