...
/*
You are given two integers m and n that represent the height and width of a rectangular 
piece of wood. You are also given a 2D integer array prices, where prices[i] = [hi, wi, pricei] 
...
width of the piece to split it into two smaller pieces. 
...
public:
    ...
    long long dfs(int row, int col, map<pair<int, int>, long long>&mp, vector<vector<long long>>&memo){
        if(memo[row][col]!=-1) return memo[row][col];
        long long ans=mp[{row, col}];
        for(int i=1; i<row; ++i){
            ans=max(ans, dfs(i, col, mp, memo)+dfs(row-i, col, mp, memo));
        }
        for(int j=1; j<col; ++j){
            ans=max(ans, dfs(row, j, mp, memo)+dfs(row, col-j, mp, memo));
        }
